...
def get_transform(opt, params=None, img_size=[512, 512], grayscale=False, method=Image.BICUBIC, convert=True):
    if 'aug' in opt.preprocess:
        ...
        if opt.random_view > 0:
            resize = random.random() * opt.random_view + (opt.crop_size)/min(img_size)
            transform_list.append(transforms.Resize((int(resize*img_size[1]), int(resize*img_size[0])), method))
