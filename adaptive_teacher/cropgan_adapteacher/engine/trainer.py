    ...
    def build_hooks(self):
        ...
        if comm.is_main_process():
            ...
            ret.append(hooks.BestCheckpointer(
                self.checkpointer,val_metric="AP50",mode='max'
            ))
        return ret
...
class ATeacherTrainer(DefaultTrainer):
    def __init__(self, cfg):
        ...
        # # Count experiments in output_dir/tensorboard to index the experiment
        # experiment_idx = len(os.listdir(f"{cfg.OUTPUT_DIR}/tensorboard"))

        self.probe = OpenMatchTrainerProbe(cfg)
        # self.writer = SummaryWriter(log_dir=cfg.OUTPUT_DIR)
        print(f"Tensorboard output will be stored at {cfg.OUTPUT_DIR}")
        self.register_hooks(self.build_hooks())  
    ...
    def build_hooks(self):
        ...
        ret.append(EvalHook(cfg.TEST.EVAL_PERIOD,
